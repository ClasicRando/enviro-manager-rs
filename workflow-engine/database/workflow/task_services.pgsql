create table if not exists workflow.task_services (
...
call audit.audit_table('workflow.task_services');

comment on table workflow.task_services is
'All services that provide remote task execution';
comment on column workflow.task_services.service_id is
'Unique identifier for each service';
comment on column workflow.task_services.name is
'Alias given to the service. Must be unique';
comment on column workflow.task_services.base_url is $$

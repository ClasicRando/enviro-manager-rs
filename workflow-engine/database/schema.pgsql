...
if current_database() like '%test%' then
    create schema if not exists workflow_run authorization we_test;
else
    create schema if not exists workflow_run authorization we_admin;
end if;
revoke all on schema workflow_run from public;
grant usage on schema workflow_run to we_web;
comment on schema workflow_run is 'Workflow Run related objects for the workflow engine';

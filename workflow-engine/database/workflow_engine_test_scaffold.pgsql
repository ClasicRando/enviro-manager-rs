...
begin
    truncate
        workflow.task_services,
        workflow.tasks,
        workflow.workflows,
        workflow.workflow_tasks,
        executor.executors,
        workflow_run.workflow_runs,
        workflow_run.task_queue
        restart identity
        cascade;

    insert into workflow.task_services as t(name, base_url)
    ...
    insert into workflow_run.workflow_runs as wr(workflow_id, executor_id)
    values(v_workflow_id, v_executor_id)
    returning wr.workflow_run_id into v_workflow_run_id;

    execute format(
        'create table if not exists "workflow_run".%I partition of workflow_run.task_queue for values in (%L)',
        'task_queue_'||v_workflow_run_id,
        v_workflow_run_id
    );

    insert into workflow_run.task_queue as tq(workflow_run_id, task_order, task_id, parameters, status)
    select
        v_workflow_run_id workflow_run_id, wt.task_order, wt.task_id,
        wt.parameters, 'Running'::workflow_run.task_status status

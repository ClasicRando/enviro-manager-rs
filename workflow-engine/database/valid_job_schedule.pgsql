...
as $$
declare
    entry workflow_engine.schedule_entry;
begin
    if job_schedule is null or job_schedule = '{}'::workflow_engine.schedule_entry[] then
        return false;
    end if;
...
comment on function workflow_engine.valid_job_schedule IS $$
